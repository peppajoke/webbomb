...
<a class="navbar-brand ' . ( $currentApp === 'home' ? '' : 'text-muted') . '" href="' . string_helper::getAppUrl() . '">' . $this->logo() . 'webBomb</a>
...
';
  ...
  private function accountManagement() {
    ...
    if ($user) {
      return '
        <p class="text-muted">Logged in as ' . $user->username .'</p>
        <a href="' . string_helper::getAppUrl('login', 'logout') . '" class="text-muted">[logout]</a>
      ';
    }
    return '
